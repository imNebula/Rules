...
为了保证规则性能及配置文件的整洁，构建脚本会在编译时进行自动去重：
- **同分组内自动去重**：如果在同一个分组内手动书写了重复的规则，或者手动编写的规则与通过 `@list`/`@yaml` 导入的外部规则重复，构建脚本会自动去重，只保留一份，且**不会报错中断**。
- **允许跨分组重复**：支持不同的分组中出现相同的域名规则（例如，同一个域名可以在多个不同的分流策略组中同时存在）。
